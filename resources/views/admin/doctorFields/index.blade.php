...
@can('doctor_field_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.doctor-fields.create') }}">
                        ...
                        <tr data-entry-id="{{ $doctorField->id }}">
                            ...
                            <td>
                                @can('doctor_field_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.doctor-fields.show', $doctorField->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('doctor_field_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.doctor-fields.edit', $doctorField->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('doctor_field_delete')
                                    <form action="{{ route('admin.doctor-fields.destroy', $doctorField->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('doctor_field_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.doctor-fields.massDestroy') }}",
