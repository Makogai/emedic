        ...
        <div class="col-md-12">
            @can('drug_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.drugs.create') }}">
                            ...
                            <tbody>
                                @foreach($drugs as $key => $drug)
                                    <tr data-entry-id="{{ $drug->id }}">
                                        ...
                                        <td>
                                            @can('drug_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.drugs.show', $drug->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('drug_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.drugs.edit', $drug->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('drug_delete')
                                                <form action="{{ route('frontend.drugs.destroy', $drug->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('drug_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.drugs.massDestroy') }}",
