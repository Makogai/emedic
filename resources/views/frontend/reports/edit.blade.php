                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.reports.update", [$report->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="title" id="title" value="{{ old('title', $report->title) }}" required>
                            @if($errors->has('title'))
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control ckeditor" name="content" id="content">{!! old('content', $report->content) !!}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control datetime" type="text" name="date" id="date" value="{{ old('date', $report->date) }}" required>
                            @if($errors->has('date'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('date') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="doctor_id" id="doctor_id" required>
                                @foreach($doctors as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('doctor_id') ? old('doctor_id') : $report->doctor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="patient_id" id="patient_id" required>
                                @foreach($patients as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('patient_id') ? old('patient_id') : $report->patient->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
